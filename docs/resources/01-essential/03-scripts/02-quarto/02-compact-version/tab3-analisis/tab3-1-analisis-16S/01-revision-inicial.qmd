...
ruta_archives_16S <- file.path(ruta_metagenomics_pipeline, "resources", "01-essential", "02-archives", "02-tmp", "tab3-analisis", "tab3-1-analisis-16S")

# Crear la carpeta si no existe
if (!dir.exists(ruta_archives_16S)) {
  dir.create(ruta_archives_16S, recursive = TRUE, showWarnings = FALSE)
}

# Ruta del archivo TXT de metadatos creado
ruta_archives_16S_sample_metadata <- file.path(ruta_archives_16S, "sample_metadata.txt")

# Creación del archivo en formato TXT
write.table(
  Def_16S_sample_metadata_relativa,
  file = ruta_archives_16S_sample_metadata,
...
cat(glue::glue('
<p>
  El presente análisis de <strong>metagenómica de amplicones 16S</strong> tiene como objetivo caracterizar la composición y diversidad de las comunidades microbianas presentes en las muestras seleccionadas, cuyo entorno de trabajo se encuentra organizado dentro de la siguiente ruta del proyecto:
</p>
...
cat(glue::glue('
<p>
  A continuación, se presenta en detalle la estructura de carpetas y archivos empleada en el análisis de metagenómica de amplicones 16S, organizada para facilitar tanto el desarrollo reproducible del análisis como la posterior consulta e interpretación de los resultados:
</p>

<div class="items">
<ul>
  <li>
    <strong><code>{nombre_proyecto}/Analisis/reads/16S/</code></strong> <strong>&rArr;</strong> Directorio que contiene los archivos de lectura crudos de las muestras, preparados para su procesamiento con QIIME2.
  </li>

  <li>
    <strong><code>{nombre_proyecto}/Resultados/Def_16S/</code></strong> <strong>&rArr;</strong> Carpeta que agrupa los resultados generados por QIIME2 y la predicción funcional de las comunidades microbianas:
    <ul>
      <li>
...
if (file.exists((ruta_Def_16S_sample_metadata_relativa))) {
  ...
  <p style="text-align:center; margin-top: 5px;">
    <a href="{ruta_Def_16S_sample_metadata_relativa}" 
      ...
      <i class="fa-solid fa-file-arrow-down"></i> Descargar archivo "{basename(ruta_Def_16S_sample_metadata_relativa)}"
    </a>
  </p>
  '), sep = "\n")
}
...
<div class="iframe-container">
  <iframe src="{ruta_archives_16S_sample_metadata}" frameborder="0" style="height: 500px;"></iframe>
</div>

<p style="text-align:center;">
  <a href="{ruta_archives_16S_sample_metadata}"
    target="_blank" 
    class="boton-iframe">
