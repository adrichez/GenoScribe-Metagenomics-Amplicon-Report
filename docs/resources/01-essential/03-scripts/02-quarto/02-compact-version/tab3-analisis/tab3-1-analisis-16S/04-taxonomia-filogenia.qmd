...
<div class="informe-titulo-tab">
  <h1>Pestaña</h1>
  <h2>Análisis Bioinformático Completo (16S)</h2>
</div>
...
ruta_my_taxonomy_qzv_relativa <- file.path(ruta_Def_16S_relativa, "my_taxonomy.qzv")
ruta_my_taxonomy_qzv <- file.path(ruta_Def_16S, "my_taxonomy.qzv")

# Definir ruta del archivo .qza
ruta_my_taxonomy_qza_relativa <- file.path(ruta_Def_16S_relativa, "my_taxonomy.qza")
ruta_my_taxonomy_qza <- file.path(ruta_Def_16S, "my_taxonomy.qza")

# Definir ruta de salida para el unzip
ruta_Def_16S_unzip <- file.path(ruta_Def_16S_relativa, "unzip")
ruta_unzip_my_taxonomy <- file.path(ruta_Def_16S_unzip, "my_taxonomy")
...
```{r}
# Definir ruta del archivo .qzv
ruta_taxa_bar_plots_qzv_relativa <- file.path(ruta_Def_16S_relativa, "taxa-bar-plots.qzv")
ruta_taxa_bar_plots_qzv <- file.path(ruta_Def_16S, "taxa-bar-plots.qzv")

# Definir ruta de salida para el unzip
ruta_Def_16S_unzip <- file.path(ruta_Def_16S_relativa, "unzip")
ruta_unzip_taxa_bar_plots <- file.path(ruta_Def_16S_unzip, "taxa-bar-plots")
...
for (f in archivos_csv) {
  ...
  cat(glue::glue('
    ...
    <div class="titulo4" id="tab3-seccion4.1.2.{nivel+1}" 
      style="margin-top: 50px; margin-bottom: 12px; 
        font-size: 23px; font-weight: 900; color: #004151; 
        text-transform: uppercase; letter-spacing: 0.5px; 
        border-bottom: none; box-shadow: 0px -1px 0px 0px #dedede inset; 
        padding-bottom: 1px; line-height: 0.6; 
        -webkit-text-stroke: 1.3px #0a6278; word-spacing: -3px;">
  ...
  if (file.exists(f)) {
    ...
    # Creamos la tabla con tus opciones nuevas
    ...
    # Imprimir la tabla
    # Usamos tagList() dentro de print() para asegurar que se renderiza el widget
    # Esto NO añade estilos ni divs extra, solo prepara el objeto para salir por pantalla
    print(htmltools::tagList(tabla_dt))
...
ruta_aligned_rep_seqs_qza <- file.path(ruta_Def_16S, "aligned-rep-seqs.qza")
ruta_aligned_rep_seqs_qza_relativa <- file.path(ruta_Def_16S, "aligned-rep-seqs.qza")

ruta_unrooted_tree_qza <- file.path(ruta_Def_16S, "unrooted-tree.qza")
ruta_unrooted_tree_qza_relativa <- file.path(ruta_Def_16S, "unrooted-tree.qza")

ruta_rooted_tree_qza <- file.path(ruta_Def_16S, "rooted-tree.qza")
ruta_rooted_tree_qza_relativa <- file.path(ruta_Def_16S, "rooted-tree.qza")
```

```{r}
# Definir ruta del archivo .qzv
ruta_unrooted_tree_qzv_relativa <- file.path(ruta_Def_16S_relativa, "unrooted-tree.qzv")
ruta_unrooted_tree_qzv <- file.path(ruta_Def_16S, "unrooted-tree.qzv")

# Definir ruta de salida para el unzip
ruta_Def_16S_unzip <- file.path(ruta_Def_16S_relativa, "unzip")
ruta_unzip_unrooted_tree <- file.path(ruta_Def_16S_unzip, "unrooted-tree")
...
if (file.exists(ruta_unrooted_tree_qzv_relativa)) {
  ...
  message("⚠️ El archivo unrooted-tree.qzv no existe. Se omite el unzip.")
}
```
...
```{r}
# Definir ruta del archivo .qzv
ruta_rooted_tree_qzv_relativa <- file.path(ruta_Def_16S_relativa, "rooted-tree.qzv")
ruta_rooted_tree_qzv <- file.path(ruta_Def_16S, "rooted-tree.qzv")

# Definir ruta de salida para el unzip
ruta_Def_16S_unzip <- file.path(ruta_Def_16S_relativa, "unzip")
ruta_unzip_rooted_tree <- file.path(ruta_Def_16S_unzip, "rooted-tree")
...
if (file.exists(ruta_rooted_tree_qzv_relativa)) {
  ...
  message("⚠️ El archivo rooted-tree.qzv no existe. Se omite el unzip.")
}
```
