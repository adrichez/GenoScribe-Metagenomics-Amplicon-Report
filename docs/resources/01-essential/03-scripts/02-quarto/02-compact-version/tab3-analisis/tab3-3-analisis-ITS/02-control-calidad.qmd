...
<!-- &&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&& -->
<!--  TAB 3: ANÁLISIS ITS | 2. CONTROL DE CALIDAD Y PREPROCESAMIENTO -->
<!-- &&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&& -->
...
ruta_Pre_fastqc_results <- file.path(params$project_path, "Analisis", "miARma_ITS", "Pre_fastqc_results")
ruta_Pre_fastqc_results_relativa <- file.path(ruta_proyecto_relativa, "Analisis", "miARma_ITS", "Pre_fastqc_results")
...
ids_targets <- Def_ITS_sample_metadata_relativa[, 1]
...
ruta_multiqc_report <- file.path(ruta_nextflow_results, "02-multiqc-report", "results-multiqc-ITS", "multiqc_report.html")
```


<div class="informe-titulo-tab">
  <h1>Pestaña</h1>
  <h2>Análisis Bioinformático Completo (ITS)</h2>
</div>
...
<div class="informe-resumen">
  ...
  <p>
    La tabla de abundancia de ASVs (<code>table.qza / table.qzv</code>) y las secuencias representativas (<code>rep-seqs.qza / rep-seqs.qzv</code>) se analizarán en la siguiente sección que detallaremos más adelante: <a href="03-asvs-abundancia.html#tab3-seccion3" class="normalink">3. Generación de ASVs y tabla de abundancia</a>.
  </p>

  <p>
    En conjunto, esta sección proporciona una visión completa del control de calidad, demultiplexación, preprocesamiento y depuración de errores aplicada a las lecturas ITS. La integración de todos estos apartados garantiza que el informe sea <strong>automatizado, reproducible</strong> y adaptable a cualquier conjunto de muestras, manteniendo siempre la trazabilidad de los datos procesados.
  </p>
</div>
...
ruta_demux_qzv_relativa <- file.path(ruta_Def_ITS_relativa, "demux.qzv")
ruta_demux_qzv <- file.path(ruta_Def_ITS, "demux.qzv")

# Definir ruta de salida para el unzip
ruta_Def_ITS_unzip <- file.path(ruta_Def_ITS_relativa, "unzip")
ruta_unzip_demux <- file.path(ruta_Def_ITS_unzip, "demux")
...
<div style="text-align: center;">
  <table style="border-collapse: collapse; margin: 0 auto; width: 60%;">
    ...
    <tbody>
      <tr>
        <td style="border: 1px solid #ccc; padding: 6px;">NGS069-24-ITS-1</td>
        <td style="border: 1px solid #ccc; padding: 6px;">146241</td>
        <td style="border: 1px solid #ccc; padding: 6px;">146241</td>
      </tr>
      <tr>
        <td style="border: 1px solid #ccc; padding: 6px;">NGS069-24-ITS-8</td>
        <td style="border: 1px solid #ccc; padding: 6px;">129633</td>
        <td style="border: 1px solid #ccc; padding: 6px;">129633</td>
      </tr>
      <tr>
        <td style="border: 1px solid #ccc; padding: 6px;">NGS069-24-ITS-9</td>
...
<ul>
  ...
  <li><strong>Importancia para ITS</strong> <strong>&rArr;</strong> un control de calidad adecuado garantiza que la inferencia de ASVs/OTUs sea confiable, evitando errores en la taxonomía y en los análisis de diversidad alfa y beta.</li>
</ul>

<p>
  En conjunto, este control de calidad integral constituye un pilar indispensable en cualquier pipeline de análisis de metagenómica ITS. La combinación de FastQC, MultiQC y la revisión de demultiplexación maximiza la confianza en los datos, reduce la introducción de artefactos y asegura que los resultados biológicos reflejen de manera precisa la composición y estructura real de las comunidades microbianas estudiadas.
</p>
...
```{r}
# Definir ruta del archivo .qzv
ruta_stats_dada2_qzv_relativa <- file.path(ruta_Def_ITS_relativa, "stats-dada2.qzv")
ruta_stats_dada2_qzv <- file.path(ruta_Def_ITS, "stats-dada2.qzv")

# Definir ruta de salida para el unzip
ruta_Def_ITS_unzip <- file.path(ruta_Def_ITS_relativa, "unzip")
ruta_unzip_stats_dada2 <- file.path(ruta_Def_ITS_unzip, "stats-dada2")
...
<ul>
  ...
  <li>
    ...
    <a href="03-asvs-abundancia.html#tab3-seccion3" class="normalink">3. Generación de ASVs y tabla de abundancia</a>. 
