...
println "Tipo de amplicón -> ${params.amplicon_type}"
...
process MULTIQC {
    ...
    // Carpeta destino
    publishDir "${params.outdir}/02-multiqc-report/results-multiqc-${tipo}", mode: 'copy'
...
workflow {
    ...
    // Determinar qué tipos lanzar según params.amplicon_type
    def tipos_a_lanzar = []
    switch(params.amplicon_type) {
    ...
    params_yml_out = CREAR_PARAMS_YML(params.project_path, params.amplicon_type, params.report_version)
    ...
    quarto_yml_out = CREAR_QUARTO_YML(params.report_version, params.amplicon_type)
